...
return function()
    ...
    function mainWidget:Update(dt)
        ...
        if (((self.cacheElapsed > self.cacheTime) or self.needData) and requestChannel:getCount() == 0) then
            ...
            requestChannel:push({type="frames", x=baseX, sX = scrollX, xo = x_offset, sw = scrollW, clickedFrame = self.clickedFrame, zoom = self.zoom})
        ...
        for lineIdx=1, self.fileLength do
            if (lineIdx % self.zoom == 0) then
                ...
                if data then
                    ...
                    self.upperBound = math.max(self.upperBound, value)
                    local scaledUnit = h / (self.upperBound * 1.1)
                    local heightRatio = math.max((value * scaledUnit), 20)
